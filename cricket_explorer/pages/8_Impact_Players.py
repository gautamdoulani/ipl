"""Cricket Data Explorer - Impact Players analysis (IPL 2023+)."""
...
# Full table
most_used_in = run_query(f"""
    SELECT
        player_in as player,
        COUNT(*) as times_used,
        COUNT(DISTINCT {team_sql}) as teams,
    ...
    LIMIT 20
""")
st.dataframe(most_used_in, width="stretch", hide_index=True)
...
most_replaced = run_query(f"""
    ...
    LIMIT 20
""")
st.dataframe(most_replaced, width="stretch", hide_index=True)

st.subheader("Recent Impact Player Substitutions")
recent_subs = run_query(f"""
    SELECT
        ip.season,
        {team_sql} as team,
        ...
        m.match_date,
        CASE WHEN m.team1 = ip.team THEN {team2_sql} ELSE {team1_sql} END as opponent
    FROM impact_players ip
    JOIN stg_matches m ON ip.match_id = m.match_id
    ORDER BY m.match_date DESC
    LIMIT 30
""")
st.dataframe(recent_subs, width="stretch", hide_index=True)
