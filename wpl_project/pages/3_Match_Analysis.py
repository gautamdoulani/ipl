"""WPL Data Explorer - Match Analysis/Scorecard page."""
...
pre_selected_match = match_id_param if match_id_param else None
...
matches = run_query(f"""
    SELECT match_id, match_date,
           REPLACE(team1, 'Royal Challengers Bangalore', 'Royal Challengers Bengaluru') || ' vs ' ||
           REPLACE(team2, 'Royal Challengers Bangalore', 'Royal Challengers Bengaluru') as match_name,
           city
    FROM stg_matches
...
if selected_match:
    ...
    innings_totals = run_query(f"""
        SELECT
            innings,
            REPLACE(batting_team, 'Royal Challengers Bangalore', 'Royal Challengers Bengaluru') as batting_team,
        ...
        GROUP BY innings, REPLACE(batting_team, 'Royal Challengers Bangalore', 'Royal Challengers Bengaluru')
        ORDER BY innings
    """)
    ...
    winner = winner_raw.replace('Royal Challengers Bangalore', 'Royal Challengers Bengaluru') if winner_raw else None
    if winner:
    ...
    toss_winner = toss_winner.replace('Royal Challengers Bangalore', 'Royal Challengers Bengaluru') if toss_winner else 'N/A'
    ...
    for innings in [1, 2]:
        innings_data = run_query(f"""
            SELECT REPLACE(batting_team, 'Royal Challengers Bangalore', 'Royal Challengers Bengaluru') as batting_team
            FROM stg_deliveries
