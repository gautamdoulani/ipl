"""WPL Data Explorer - Player Profile page."""
...
def get_all_players():
    return run_query("""
        ...
        agg AS (
            SELECT name, SUM(balls) as total_balls
            FROM player_balls
            GROUP BY name
        )
        SELECT a.name, p.espn_id, a.total_balls
        FROM agg a
        LEFT JOIN people p ON a.name = p.name
        ORDER BY a.total_balls DESC
    """)
...
if selected_player:
    ...
    with col1:
        ...
        display_player_image(photo_url, espn_id if photo_url else None, size=120)
