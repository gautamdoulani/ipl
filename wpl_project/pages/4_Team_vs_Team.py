"""WPL Data Explorer - Team vs Team comparison."""
...
if team1 and team2 and team1 != team2:
    ...
    team_stats = run_query(f"""
        SELECT
            REPLACE(team, 'Royal Challengers Bangalore', 'Royal Challengers Bengaluru') as team,
            matches_played, matches_won, win_percentage,
            team_strike_rate, team_economy_rate
        FROM team_metrics
        WHERE REPLACE(team, 'Royal Challengers Bangalore', 'Royal Challengers Bengaluru') IN ('{team1}', '{team2}')
    """)
    ...
    recent['Scorecard'] = recent.apply(
        lambda row: f"/Match_Analysis?match_id={row['match_id']}&season={row['season']}",
        axis=1
    )
