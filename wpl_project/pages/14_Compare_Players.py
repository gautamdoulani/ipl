"""WPL Data Explorer - Compare two players' overall stats side by side."""
...
players = run_query("""
    SELECT DISTINCT name FROM (
        SELECT batter as name FROM batting_metrics WHERE total_balls >= 50
        UNION
        SELECT bowler as name FROM bowling_metrics WHERE total_balls >= 50
    )
    ORDER BY name
""")
...
if player1 and player2:
    ...
    player_info = run_query(f"""
        SELECT name, espn_id,
               'https://a.espncdn.com/i/headshots/cricket/players/full/' || espn_id || '.png' as photo_url
        FROM people
    ...
    for idx, (col, player) in enumerate([(col1, player1), (col2, player2)]):
        with col:
            ...
            if len(info) > 0:
                display_player_image(info['photo_url'].iloc[0], info['espn_id'].iloc[0], size=120)
